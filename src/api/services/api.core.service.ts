...
export class CoreService {
    ...
    async getDocument(document: string, id: string, fields?: string[], excludeFields?: string[]) {
        ...
        const res = await this.getDocuments(document, options);
        return res?.[0];
    }

    async getSingleDocument(document: string, options: DBOption) {
        const res = await this.getDocuments(document, options);
        return res?.[0];
    }

    async getDocuments(document: string, options: DBOption) {
    ...
    async createDocument(document: string, doc: any) {
        const req:any = {}//getFromLocalStorage<SRequest>("request")!;
        const user = req.user;
        ...
        await this.updateChildTable(docType, doc, mysqlConn);
    ...
    async updateDocument(document: string, doc: any, filter?: DBFilter) {
        const req:any = {}// getFromLocalStorage<SRequest>("request")!;
        const user = req.user;
        const mysqlConn = req.mysqlConn!;
    ...
    async deleteDocument(document:string, filter:DBFilter,permanentDelete=false) {
         const req:any = {}// getFromLocalStorage<SRequest>("request")!;
        const user = req.user;
        const mysqlConn = req.mysqlConn!;
    ...
    async selfOnlyFilter(document: string, filter: DBFilter | undefined, user: any, mysqlConn: ConnectionAction) {
    ...
    async filterDeleted(docType: MyERPDocType, filter: DBFilter | undefined, mysqlConn: ConnectionAction) {
        filter = filter ?? [];
        try {
    ...
    async filterSysAndCom(document: string, filter: DBFilter | undefined, sys: string | undefined, com: string | undefined, mysqlConn: ConnectionAction, docType?: MyERPDocType) {
        filter = filter ?? [];
        if (!docType) {
            try {
    ...
    async getTableFields(document: string, docType?: MyERPDocType, nameOnly = false): Promise<MyERPField[] | string[]> {
    ...
    async getLinkData(link: any, linkDocType: MyERPDocType, language: string, mysqlConn: ConnectionAction): Promise<any> {
        ...
        for (let l of links) {
            ...
            const linkData: any = (await this.getData(options)).data;
    ...
    async sqlUpdate(document: string, doc: any, sqlWhere: string, mysqlConn: ConnectionAction) {
    ...
    async updateChildTable(parentDocType: MyERPDocType, parentDoc: any, mysqlConn: ConnectionAction) {
        ...
        for (const child of childTables) {
            ...
            for (const childDoc of childDocs) {
                if (childDoc.id) {
                    ...
                    await this.updateDocument(childTableName, childDoc, filter);
                } else {
                    childDoc[parentField!.id] = parentDoc[parentField?.parentField];
                    await this.createDocument(childTableName, childDoc);
                }
