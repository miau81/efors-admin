...
export class ApiEInvoiceService {

    async getAccessToken(setting:EinvoiceSetting) {
        ...
        const url = `${this.getEInvoiceEnvironment(setting.isSandBox).api}/connect/token`;
    ...
    async validateTaxPayerTin(taxPayer: EInvoiceTaxPayer, setting:EinvoiceSetting, token?: string) {
        if (!token) {
            token = await this.getAccessToken(setting);
        }
        http.setHeader('Content-Type', 'application/json');
        http.setHeader('Authorization', token);
        const url = `${this.getEInvoiceEnvironment(setting.isSandBox).api}/api/v1.0/taxpayer/validate/${taxPayer.tinNo}`;
    ...
    //     http.setHeader('Content-Type', 'application/json');
    //     http.setHeader('Authorization', token);
    //     const url = `${this.getEInvoiceEnvironment(isSandBox).api}/api/v1.0/documentsubmissions/${submissionUid}`;
    ...
    //     http.setHeader('Content-Type', 'application/json');
    //     http.setHeader('Authorization', token);
    //     const env = this.getEInvoiceEnvironment(isSandBox);
    ...
    //         http.setHeader('Content-Type', 'application/json');
    //         http.setHeader('Authorization', token);
    //         const url = `${this.getEInvoiceEnvironment(isSandBox).api}/api/v1.0/documentsubmissions`;
    ...
    private getEInvoiceEnvironment(isSandBox: boolean) {
    ...
    //                 Party: [{
    //                     IndustryClassificationCode: [{ _: invoice.supplier.industryClassificationCode, name: invoice.supplier.businessActivity }],
    //                     PartyIdentification: [
...
export interface EInvoiceTaxPayer {
    idType: string;
    idValue: string;
    tinNo: string;
}
