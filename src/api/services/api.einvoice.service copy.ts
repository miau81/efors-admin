...
export class EInvoiceService {
    ...
    async getAccessToken(clientId: string, clientSecret: string, isSandBox: boolean) {
        ...
        const url = `${this.getEInvoiceEnvironment(isSandBox).api}/connect/token`;
    ...
    async validateTaxPayerTin(taxPayer: EInvoiceTaxPayer, clientId: string, clientSecret: string, isSandBox: boolean, token?: string) {
        if (!token) {
            token = await this.getAccessToken(clientId, clientSecret, isSandBox);
        }
        http.setHeader('Content-Type', 'application/json');
        http.setHeader('Authorization', token);
        const url = `${this.getEInvoiceEnvironment(isSandBox).api}/api/v1.0/taxpayer/validate/${taxPayer.tinNo}`;
